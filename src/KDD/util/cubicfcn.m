...
% Assign answers -------------------------------------------------------
if (DD < 0.),x=[y1 y2 y3];
elseif (DD == 0.) x=[y1 y2r];
else x=[y1];
end

x=x(x>0);
x=min(x);
if(length(x)==0)error('no good step');end;
if(x==0)error('zero step');end;



function [x]=quadfcn(a, b, c)
% ----------------------------------------------------------------------
% Usage:       [x]=quadfcn(a, b, c)
%
